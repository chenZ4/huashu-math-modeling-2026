...
Q1 结果附表 & q1/latex/q1\_tables.pdf & 定稿汇总/与冻结对比/λ/轮次/t2-div 五表 \\
Q2 结果附表 & q2/latex/q2\_tables.pdf & 定稿汇总/与冻结对比/t2/死区/轮次分布 五表 \\
Q3 结果附表 & q3/latex/q3\_tables.pdf & 二分判定过程/d*与HPWL/临界点扰动核验 三表 \\
\multicolumn{3}{l}{\footnotesize 全部图表已按题目分类集中至 \texttt{图表/}（Q1-Q4 + 公共/），论文直接引用} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{灵敏度图}：\texttt{图表/灵敏度/QN/}（12 张，按题目分类，
    每问含图 + S 系列数据 CSV）；
    \item \textbf{EDA 图}：\texttt{eda/figs/*.png}（8 张，数据规模/尺寸分布/线网统计）；
    \item \textbf{总览图}（\texttt{图表/公共/总体结论/}）：
    \texttt{overall\_improvement.png}（三问 vs 冻结提升总览，建议放摘要或结果章首）、
    \texttt{overall\_runtime.png}（四问耗时对比）；Q2 8 轮箱线图在
    \texttt{图表/Q2/图/q2\_rounds\_distribution.png}。
\end{itemize}
...
\begin{itemize}
    ...
    可按其中公式与结构直接组织论文章节}；
    \item \textbf{图表/灵敏度/QN/qN\_sensitivity\_report.pdf}：各问
    灵敏度分析报告（LaTeX 源同目录可改，参数曲线 + 结论）；
    \item \textbf{paper/sensitivity\_analysis.pdf}（12 页）：论文
    "参数灵敏度分析"章节的现成成稿（12 图 + 分析文字 + 参数选择汇总表）；
    \item \textbf{paper/model\_evaluation.pdf}（3 页）：论文
    "模型评价与推广"章节成稿（评价指标体系表 + 优点 6 条 + 局限 6 条 +
    推广 6 条，全部证据带数值出处）；
    \item \textbf{paper/eda\_analysis.pdf}（6 页）：论文"数据分析"
    章节成稿——数据总览（三维度）+ 	extbf{四个问题分别分析}：
    Q1 几何异质性（CV/长条模块）、Q2 网表集中性与枢纽、Q3 轮廓与
    利用率下界、Q4 凹角空缺量化，每问配图/表/结论话术；
    \item \textbf{paper/model\_assumptions.pdf}（3 页）：论文
    "模型假设"章节成稿（通用假设 6 条 + 四问分别假设及合理性说明 +
    假设敏感性表）；
    \item \textbf{paper/appendix\_core\_methods.pdf}（5 页）：核心
    算法与方法实现附录（B$^*$ 树编码/完备性、Skyline 解码伪代码、
    扰动算子、快速模拟退火全流程、Q2 两阶段、Q3 二分与双向确认、
    Q4 超块穷举、复现协议与复杂度表），正文后直接附；
