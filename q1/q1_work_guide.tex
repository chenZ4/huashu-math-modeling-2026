...
% 定稿（2026-08-10）：q1/output/final/q1_metrics.csv 与 data/raw 重算。
% 面积利用率 = T/A、死区比例 = 1 - T/A、下界相对差距 = (A-T)/T、
% 宽高比 R = max(W,H)/min(W,H)。

\begin{table}[H]
\centering
\caption{Q1 三芯片定稿结果（$\lambda = 0.5$，n200 取 t2-div=30，固定种子多轮取优）}
\footnotesize
...
\midrule
n100 & $414\times457$ & 189198 & 179501 & 0.9487 & 5.13\% & 5.40\% & 1.104 & 0.8 \\
n200 & $376\times487$ & 183112 & 175696 & 0.9595 & 4.05\% & 4.22\% & 1.295 & 11.4 \\
n300 & $417\times684$ & 285228 & 273170 & 0.9577 & 4.23\% & 4.41\% & 1.640 & 19.3 \\
\bottomrule
\end{tabular}
\end{table}

注：面积利用率 $\eta = T/A$（$T$ 为模块总面积，面积下界）；死区比例
$= 1 - \eta$；下界相对差距 $= (A - T)/T$。定稿数值经交叉核对与挂机
scan best 逐位一致；三组芯片均通过独立合法性复核（无重叠、尺寸保持）。

\begin{table}[H]
\centering
\small
\caption{Q1 定稿 vs 冻结交付的改善（面积优先，同面积比长宽比）}
\begin{tabular}{lcccc}
\toprule
芯片 & 冻结面积 & 定稿面积 & 较冻结提升 & 长宽比 \\
\midrule
n100 & 189198 & 189198 & $0\%$ & 1.104 \\
n200 & 183840 & \textbf{183112} & $-0.40\%$ & 1.295 \\
n300 & 287448 & \textbf{285228} & $-0.77\%$ & 1.640 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{面积利用率与死区}：三组芯片利用率均 $> 94\%$（死区比例
    $< 6\%$）。理论上界 $\eta = 1$ 仅对极特殊尺寸集合可达，故 $\eta > 0.94$
    已接近工程极限；剩余死区面积 n100 / n200 / n300 分别为 9697 / 7416 /
    12058，占总面积的 5.13\% / 4.05\% / 4.23\%；
    \item \textbf{长宽比}：按题目标准（面积优先、同面积比长宽比）全部达标；
    n100 最接近正方形（1.104），n300 最长条（1.640，面积最优先下
    长宽比让位于面积）；
...
\begin{itemize}
    ...
    \item \texttt{q1/visualization/figs/final/<chip>/}：每芯片
