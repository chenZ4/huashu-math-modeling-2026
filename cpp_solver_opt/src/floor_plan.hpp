...
public:
  ...
  void init(vector<BLOCK>& blcks) {
    ...
    vector<ID> cy;
    cy.reserve(_nodes.size());
    cy.push_back(root_id);
    size_t cur = 0;
...
private:
  bool randb() const { return rand() % 2; }
  void dfs(ID id, vector<ID>& cy, size_t& cur,
           vector<BLOCK>& blcks) {
    if (_nodes[id]._l()) {
  ...
  LEN find_max_y(vector<ID>& l, size_t& cur,
                 vector<BLOCK>& blcks, BLOCK& blck) {
    LEN y = 0;
    size_t it = cur, rit = cur;
    while (it < l.size() && blcks[l[it]]._x < blck._x + blck._w) {
      if (blcks[l[it]]._y + blcks[l[it]]._h > y) y = blcks[l[it]]._y + blcks[l[it]]._h;
      if (blcks[l[rit]]._x + blcks[l[rit]]._w <= blck._x + blck._w) ++rit;
      ++it;
    }
    l.erase(l.begin() + cur, l.begin() + rit);
    l.insert(l.begin() + cur, blck._id);
    return y;
  }
